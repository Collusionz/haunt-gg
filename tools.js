...
(function () {
  ...
  var DM_ALLOW = {
    nigga: ['nega*', 'niger'],
    sex: ['essex*', 's**t*', '53x', 'sec*'],
    segs: ['essex*', 's**t*', '53x', 'sec*'],
    retard: ['restart*'],
    steamcommunity: ['steamcommunity*'],
    links: ['*https://tenor.com/*', '*https://discord.com/*', '*https://giphy.com/*', '*https://klipy.com/*']
  };
  function dmToJs(pat) {
  ...
  function dmGen() {
    ...
    for (var g = 0; g < tokens.length; g++) {
      var k = tokens[g].toLowerCase();
      if (DM_ALLOW[k]) for (var h2 = 0; h2 < DM_ALLOW[k].length; h2++) if (extra.indexOf(DM_ALLOW[k][h2]) === -1) extra.push(DM_ALLOW[k][h2]);
    }
    var all = extra.concat(sug.filter(function (w) { return extra.indexOf(w) === -1; }));
    $('dmExc').value = all.join(', ');
    $('dmExcNote').textContent = all.length
      ? 'Auto-suggested ' + all.length + ' allowed-term' + (all.length === 1 ? '' : 's') + ' — add them to the automod allowed-terms list.'
  ...
  function wire() {
    ...
    function normalizePastedFile(f) {
      if (!f.name || f.name === 'image.png' || f.name === 'blob') {
        ...
        try {
          ...
          f.name = 'pasted-' + ts + '.' + ext;
        }
      }
      return f;
    }

    function clipImageFiles(e) {
      ...
      if (dt.items && dt.items.length) {
        for (i = 0; i < dt.items.length; i++) {
          if (dt.items[i].kind === 'file' && dt.items[i].type && dt.items[i].type.indexOf('image') === 0) {
            ...
            if (f) out.push(normalizePastedFile(f));
          }
        }
      }
      if (!out.length && dt.files && dt.files.length) {
        for (i = 0; i < dt.files.length; i++) {
          if (dt.files[i].type && dt.files[i].type.indexOf('image') === 0) {
            out.push(normalizePastedFile(dt.files[i]));
          }
        }
      }
      return out;
    ...
    function flashZone(el) {
      ...
      setTimeout(function () { el.classList.remove('drag'); }, 350);
    }

    document.addEventListener('paste', function (e) {
      var imgs = clipImageFiles(e);
      if (!imgs.length) return;

      var tabCe = $('tab-ce');
      var tabImg = $('tab-img');

      if (tabCe && tabCe.classList.contains('active')) {
        e.preventDefault();
        cpLoadFile(imgs[0]);
        flashZone($('cpDrop'));
        return;
