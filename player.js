/* Site-wide music player. Keeps the same track playing across pages via
   sessionStorage resume; enter overlay only on a direct home load. */
(function () {
  'use strict';
  ...
  // navigation back to home (same-origin referrer) must never show it again.
  var isHome = location.pathname === '/' || location.pathname === '/index.html';
  var fromSite = false;
  try { fromSite = new URL(document.referrer).origin === location.origin; } catch (e) {}
  var showEnter = isHome && !fromSite;

  var unlocked = false;
  try { unlocked = sessionStorage.getItem('hzUnlocked') === '1'; } catch (e) {}
  ...
  // First real user gesture on any page lets audio unmute when autoplay was blocked.
  function kick() {
